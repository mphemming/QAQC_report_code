...
os.chdir('C:\\Users\\mphem\\OneDrive - UNSW\\Work\\QC_reports\\Code\\LTSPs\\python-aodntools-master\\')
...
def main_aggregator(files_to_agg, var_to_agg, site_code, input_dir='', output_dir='./',
    ...
    for index, file in enumerate(files_to_agg):
        ...
        nc = xr.open_dataset(os.path.join(input_dir, file))
    ...
    ## set global attrs 
    # Michael changed to integer to work for now
    time_start = num2date(np.min(TIME[:]).astype(int), time_units, time_calendar).strftime(TIMESTAMP_FORMAT)
    time_end = num2date(np.max(TIME[:]).astype(int), time_units, time_calendar).strftime(TIMESTAMP_FORMAT)
    time_start_filename = num2date(np.min(TIME[:]).astype(int), time_units, time_calendar).strftime(DATESTAMP_FORMAT)
    time_end_filename = num2date(np.max(TIME[:]).astype(int), time_units, time_calendar).strftime(DATESTAMP_FORMAT)
    ...
    os.close(fd) # added to close temp file before moving
