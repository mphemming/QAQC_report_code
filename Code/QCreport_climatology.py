...
# %% -----------------------------------------------------------------------------------------------
# Function to get daily climatology and plotting functions
# __________________________________________________________________________________________________
# __________________________________________________________________________________________________
# __________________________________________________________________________________________________
    ...
    if 'depth-averaged' not in str(depths):
        for n in range(len(depths)):   
            ...
            if check.sum() != 0:
                ...
                lower = np.concatenate([np.array(MNsOut[n]), np.array([MNsOut[n][0]])]) - np.concatenate([np.array(STDsOut[n]), np.array([STDsOut[n][0]])])
                plt.plot(calpositions,lower,c='r')   
                plt.plot(calpositions,np.concatenate([np.array(MAXsOut[n]), np.array([MAXsOut[n][0]])]),label='Maximum')
                plt.plot(calpositions,np.concatenate([np.array(MINsOut[n]), np.array([MINsOut[n][0]])]),label='Minimum')
        ...
        ax = plt.gca()
        # plot data during ddeployment
        check = np.logical_and(TIME >= DepDates[0],
                            TIME < DepDates[1])
        plt.plot(YEARDAY[check],VAR[check],'.',c='gray',label='Deployment')
        calpositions = np.array([1,32,60,91,121,152,182,213,244,274,305,335,365])
        for n_mon in np.arange(1,13,1):
        ...
        if np.isfinite(data).sum() != 0:
            plt.plot(calpositions,np.concatenate([np.array(MNsOut[0]), np.array([MNsOut[0][0]])]),label='Mean Climatology')
            upper = np.concatenate([np.array(MNsOut[0]), np.array([MNsOut[0][0]])]) + np.concatenate([np.array(STDsOut[0]), np.array([STDsOut[0][0]])])
            plt.plot(calpositions,upper,c='r',label='Standard Deviation Climatology')   
            lower = np.concatenate([np.array(MNsOut[0]), np.array([MNsOut[0][0]])]) - np.concatenate([np.array(STDsOut[0]), np.array([STDsOut[0][0]])])
            plt.plot(calpositions,lower,c='r')   
            plt.plot(calpositions,np.concatenate([np.array(MAXsOut[0]), np.array([MAXsOut[0][0]])]),label='Maximum')
            plt.plot(calpositions,np.concatenate([np.array(MINsOut[0]), np.array([MINsOut[0][0]])]),label='Minimum')
...
def compareVelEllipse(t,yearday,U,V,site_name,plots_dir,deployment_file_date_identifier,DepDates):

    # get U, V for deployment
    c = np.logical_and(t >= DepDates[0],
                       t <= DepDates[1])
    UDep = U[c]; VDep = V[c]; tDep = t[c]; ydDep = np.array(yearday)[c]
    # get U,V for deployment period over all years
    Dep_unYDs = np.unique(ydDep)
    mask = np.isin(yearday, Dep_unYDs)  # create boolean mask
    tSel = np.array(t)[mask]  # use mask to select corresponding values
    USel = np.array(U)[mask] 
    VSel = np.array(V)[mask] 
    # ensure to use data outside of deployment for long term stats
    c = np.logical_or(tSel < DepDates[0],
                      tSel > DepDates[1])
    tSel = tSel[c]; USel = USel[c]; VSel = VSel[c];
    ###################################################
    # Long term 
    # Calculate mean and standard deviations
    u_meanSel = np.nanmean(USel)
    v_meanSel = np.nanmean(VSel)
    u_std = np.nanstd(USel)
    v_std = np.nanstd(VSel)
    corr_coef = ma.corrcoef(ma.masked_invalid(U), ma.masked_invalid(V))[0, 1]
    # Calculate ellipse parameters
    thetaSel = np.arctan2(v_meanSel, u_meanSel)
    theta_degreesSel = np.degrees(thetaSel)
    aSel = 2 * np.sqrt(2) * np.sqrt((u_std**2 + v_std**2 + np.sqrt((u_std**2 - v_std**2)**2 + 4*corr_coef**2*u_std**2*v_std**2)) / 2)
    bSel = 2 * np.sqrt(2) * np.sqrt((u_std**2 + v_std**2 - np.sqrt((u_std**2 - v_std**2)**2 + 4*corr_coef**2*u_std**2*v_std**2)) / 2)
    ###################################################
    # Deployment 
    # Calculate mean and standard deviations
    u_meanDep = np.nanmean(UDep)
    v_meanDep = np.nanmean(VDep)
    u_std = np.nanstd(UDep)
    v_std = np.nanstd(VDep)
    corr_coef = ma.corrcoef(ma.masked_invalid(U), ma.masked_invalid(V))[0, 1]
    # Calculate ellipse parameters
    thetaDep = np.arctan2(v_meanDep, u_meanDep)
    theta_degreesDep = np.degrees(thetaDep)
    aDep = 2 * np.sqrt(2) * np.sqrt((u_std**2 + v_std**2 + np.sqrt((u_std**2 - v_std**2)**2 + 4*corr_coef**2*u_std**2*v_std**2)) / 2)
    bDep = 2 * np.sqrt(2) * np.sqrt((u_std**2 + v_std**2 - np.sqrt((u_std**2 - v_std**2)**2 + 4*corr_coef**2*u_std**2*v_std**2)) / 2)

    # In the context of calculating a velocity ellipse, a and b are the major and minor axes of the ellipse, 
    # respectively. They are calculated using the standard deviations of the eastward and northward velocities 
    # (u_std and v_std) and their correlation coefficient (corr_coef). The formulas used to calculate a 
    # and b are based on the standard equation of an ellipse, which has the form 
    # ((x - x0)**2/a**2) + ((y - y0)**2/b**2) = 1, where (x0, y0) is the center of the ellipse.

    # Create velocity ellipse
    fig, ax = plt.subplots()
    ellipse1 = Ellipse(xy=(u_meanSel, v_meanSel), width=aSel, height=bSel, angle=theta_degreesSel)
    ellipse2 = Ellipse(xy=(u_meanDep, v_meanDep), width=aDep, height=bDep, angle=theta_degreesDep)
    ellipse1.set_facecolor('none')
    ellipse1.set_edgecolor('k')
    ellipse2.set_facecolor('none')
    ellipse2.set_edgecolor('g')
    ax.add_artist(ellipse1)
    ax.add_artist(ellipse2)
    # Add arrow indicating velocity direction
    ax.quiver(0, 0, u_meanSel, v_meanSel, angles='xy', scale_units='xy', scale=1)
    ax.quiver(0, 0, u_meanDep, v_meanDep, angles='xy', scale_units='xy', scale=1,color='g')
    # Set axis limits
    Vlim = np.round(np.nanmean([np.abs(v_meanSel),np.abs(v_meanDep)]),1)
    ax.set_xlim([Vlim*-2,Vlim*2])
    ax.set_ylim([Vlim*-6,Vlim*6])
    # Set axis labels
    ax.set_xlabel('U [m s$^{-1}$]')
    ax.set_ylabel('V [m s$^{-1}$]')
    # add quiver annotations
    magSel = np.round(np.sqrt(u_meanSel**2 + v_meanSel**2),3)
    magDep = np.round(np.sqrt(u_meanDep**2 + v_meanDep**2),3)
    ax.annotate(('Historical deployment period: ' + str(magSel) + ' m s-1'), xy=(Vlim*-1.5, Vlim*5), xytext=(Vlim*-1.5, Vlim*5), fontsize=12)
    ax.annotate(('Deployment: ' + str(magDep) + ' m s-1'), xy=(Vlim*-1.5, (Vlim*5)-0.2), xytext=(Vlim*-1.5, (Vlim*5)-0.2), fontsize=12, c='g')
    plt.grid()
    # Show plot
    plt.show()
    # save figures
    filename = (plots_dir + 'VelEllipses\\VEl_Ellipse_' + site_name + '_' + deployment_file_date_identifier + '_DepthAveraged.png')
    plt.savefig(filename,dpi=300)
    plt.close()
...
# %% Create Velocity ellipse comparison plot
#####################################################
# 
